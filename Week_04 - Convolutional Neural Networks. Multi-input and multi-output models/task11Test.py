...
class OmniDataset(Dataset):
    def __init__(self, path, transform):
        ...
        all_character_labels = set()

        # Събираме всички raw character labels
        temp_samples = []
            ...
            for character in os.listdir(alphabet_path):
                ...
                for img_path in os.scandir(character_path):
                    if img_path.is_file() and img_path.name.endswith('.png'):
                        ...
                        if character_label != -1:
                            temp_samples.append((img_path.path, alphabet, character_label))
                            all_character_labels.add(character_label)

        # Създаваме мапинг на character label → индекс от 0 до N-1
        self.character_label_to_idx = {
            label: idx for idx, label in enumerate(sorted(all_character_labels))
        }
        self.num_characters = len(self.character_label_to_idx)

        # Попълваме финалния списък със (img, alphabet, character_idx)
        for img_path, alphabet, raw_label in temp_samples:
            mapped_label = self.character_label_to_idx[raw_label]
            self.samples.append((img_path, alphabet, mapped_label))

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        image_path, alphabet_name, character_idx = self.samples[idx]

        img = Image.open(image_path).convert("L")
        img = self.transform(img)

        alphabet_idx = self.alphabet_to_idx[alphabet_name]
        return img, alphabet_idx, character_idx
